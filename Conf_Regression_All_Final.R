...
fit <- lm(y ~ -1 + x, data = dataf);												#linear least square regression, through zero

newx <- seq(min(dataf$x), max(dataf$x), length.out=100)			#conf. interval
preds <- predict(fit, newdata = data.frame(x=newx), 
                 interval = 'confidence')

postscript("CalCurve.eps", horizontal = TRUE, onefile = FALSE, paper = "special", height = 8.26771654, width = 11.6929134)

plot(y ~ x, data = dataf, type = 'n', main="Calibration curve and confidence interval for Sucrose", xlab="Conc. in ppm", ylab="Peakarea", axes=FALSE)
axis(1, pos=0)
axis(2, pos=0)
...
png("CalCurve.png", height = 2480, width = 3508, res = 300)
