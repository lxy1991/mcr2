...
{
  if(name != FALSE){
    path <- paste("T:/Technik/Spektroskopie/UserData_p-mbe/IC 940 Professional Vaio/Project/Sugars/DecompositionExperiments/3rdDecomp", name, ".csv", sep="")
...
doPCA <- function(data = FALSE)
{
  if(data == FALSE){
    ...
    dTmat <- as.data.frame(Tmat);
    ############################################ -> read in as data.frame (better to handle)
    
    pca.mod <- prcomp(dTmat, retx = T, center = F);
...
plotLoadingsPCA <- function(PC1 = F, PC2 = F, PC3 = F, model)
{
  ...
  if(PC1 != F && PC2 != F && PC3 != F){
    ...
    if(PC1 != F && PC2 != F && PC3 != T){
      pcs  <- c(PC1, PC2)
      s.pcs <- sort(pcs)
      plot(model$rotation[,PC1:PC2], type = "l", main = "Loading plot", xlab = paste("PC", PC1), ylab = paste("PC", PC2))
    }
    else{
...
plotScoresPCA <- function(PC1 = F, PC2 = F, PC3 = F, model)
{
  ...
  if(PC1 != F && PC2 != F && PC3 != F){
    ...
    if(PC1 != F && PC2 != F && PC3 != T){
      pcs  <- c(PC1, PC2)
      s.pcs <- sort(pcs)
      plot(model$x[,PC1:PC2], type = "l", main = "Score plot", xlab = paste("PC", PC1), ylab = paste("PC", PC2))
    }
    else{
      if(PC1 != F && PC2 !=T && PC3 != T){
        wave <- seq(1,dim(model$rotation)[2],by=1)
        plot(wave, model$x[,PC1], main ="Score plot", xlab = "Samples", ylab = paste("PC", PC1), type = "l")
      }
    }
  }
  ############################################ -> when user has assigned 3 PCs, then do 3d plot otherwise use 2d plot
