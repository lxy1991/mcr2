Pfad <- "T:/Technik/Spektroskopie/UserData_p-mbe/IC 940 Professional Vaio/Project/Sugars/5-CalibrationRaffinose.csv";   #path in which the csv with x and y values are
...
plot(y ~ x, data = dataf, type = 'n', main="Calibration curve and confidence interval for Raffinose", xlab="Conc. in ppm", ylab="Peakarea", axes=FALSE)
axis(1, pos=0)
axis(2, pos=0)
...
text(70,1500, paste("y = ", round(fit$coeff[1],2),"* x   R = ",round(cor(calDat$x,calDat$y),5)));
confint(fit)
