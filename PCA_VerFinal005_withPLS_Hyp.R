...
readDataHyp <- function(name = FALSE)
{
  ...
  if(ua == 1){
    ...
    smplNo <- dim(spc)[2];
    cat(paste("File read ... dimensions of data file ...", smplNo, "variables,", dim(spc)[1],"samples. Use model <- doPCA(YourData) for further investigation or use plsr ..."))
    cat(sep = "\n")
    cat(sep = "\n")
    ...
    spc.hyper <- new("hyperSpec", wavelength = wl, spc = spc, labels = list(.wavelength =   "Wavelength in nm", spc = "Absorbance"), data = data.frame(sample = rownames(spc)))
    ############################################ -> read csv into pca.spc
...
doPCA <- function(data = FALSE)
{
  if(typeof(data) == "S4"){
    ...
    pca.mod <- prcomp(pca.hyp, retx = TRUE, center = FALSE);
